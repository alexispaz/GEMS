...
integer,public,parameter      :: ncsym=5    ! numero de caracteres para el nombre de un atomo
character(1),public,parameter :: csym='5'   !
                             
type, public :: element
   character(ncsym)   :: sym='A' ! Use always adjustl
   real(dp)           :: carga=0._dp
   real(dp)           :: mass=1._dp
...
function inq_z(sym)
  ...
  character(ncsym)          :: l1,l2

  ! Check string length 
  call werr('Atomic symbol too long',len_trim(adjustl(sym))>ncsym)

  l1 = trim(adjustl(sym))
  call locase(l1)
  do inq_z = 1, elements%size
    l2 = trim(adjustl(elements%o(inq_z)%sym))
    call locase(l2)
    ! print *, l2,l1,l1==l2
    if (l1==l2)  return
  enddo  
...
subroutine set_z(z,sym,mass,carga,rad)
  integer,intent(in)       :: z
  character(ncsym),optional, intent(in) :: sym
  real(dp),optional,intent(in)      :: mass,carga,rad

  if(present(sym)) elements%o(z)%sym=sym
